function [weighted_cost, exceeded_angle, control_effort, theta_error_integral, settling_time, overshoot, cart_deviation] = compute_control_metrics(params, T, X, U)
    % Compute various performance metrics for control evaluation
    
    %% Extract theta and cart position
    ...
    weighted_cost = 0.1*theta_error_integral(end) + 1*control_effort(end) + 0.1 * cart_deviation(end);
end
