function [weighted_cost, exceeded_angle, control_effort, theta_error_integral, settling_time, overshoot, cart_deviation, energy_expended] = compute_control_metrics(T, X, U, params)
    ...
    weighted_cost = 0.1*theta_error_integral(end) + 1*control_effort(end) ... 
        + 0.5*energy_expended(end) + 0.5 * cart_deviation(end) ...
        + 0.25*overshoot + 0.5*settling_time;

    weighted_cost = 0*theta_error_integral(end) + 1*control_effort(end) + ...
        0*energy_expended(end) + 0 * cart_deviation(end);
end
